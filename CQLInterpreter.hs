...
main = do
  ...
  case args of
    ...
    _ -> do
      putStrLn "Usage: cql-interpreter [-f filename] [-q query]"
      putStrLn "  -f filename   Execute the query in the specified file"
      putStrLn "  -q query      Execute the specified query"
      putStrLn "  no arguments  Start in interactive mode"
